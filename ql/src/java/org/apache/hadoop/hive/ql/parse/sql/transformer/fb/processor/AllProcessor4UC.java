...
/**
 * Process uncorrelated ALL in WHERE subquery<br>
 * AllProcessor4UC.
 *
 */
public class AllProcessor4UC extends CommonFilterBlockProcessor {
  private static final Log LOG = LogFactory.getLog(AllProcessor4UC.class);
  ...
  void processFB() throws SqlXlateException {
    ...
    fb.setASTNode(reverseCompare);

    LOG.info("Transform ALL to NOT EXISTS:"
        + reverseCompare.toStringTree().replace('(', '[').replace(')', ']'));
